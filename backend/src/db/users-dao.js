...
export async function insertArticle(articleData) {
  ...
  const sql =
    "INSERT INTO Articles (article_content, article_title, author_name ,date_published , likes, dislikes) VALUES (?, ?, ?, date('now') , 0 , 0)";
...
export async function updateUser(publisher_id, updateData) {
  ...
  const dbResult = await db.run(
    sql,
    validated.article_title,
    validated.article_content,
    publisher_id
  );
