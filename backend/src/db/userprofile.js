...
export async function getUserProfile(id) {
  ...
  const sql =
    "SELECT username, firstName, lastName, birthday, Introduction, profilePicture FROM Users where id = ? ";
...
export async function updateUserProfile(id, updateData) {
  ...
  const sql =
    "UPDATE Users SET username =?, firstName= ?, lastName = ?,birthday =?, Introduction = ?, profilePicture = ? WHERE id = ?";
  const db = await getDatabase();
